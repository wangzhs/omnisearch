...
def test_risk_flag_status_propagates_partial_inputs() -> None:
    ...
    assert risk_status.source_metadata.selection_reason == "risk_flags_rollup"


def test_signals_status_uses_helper_specific_selection_reason() -> None:
    service = StockDataService(repository=type("Repo", (), {})())

    risk_status = service._build_risk_flags_status(
        financial_status=service._build_data_status(source="tushare", updated_at="2099-01-01T00:00:00Z", cache_hit=True),
        price_status=service._build_data_status(source="akshare", updated_at="2099-01-01T00:00:00Z", cache_hit=True, partial=True, error_message="price retry pending"),
        event_status=service._build_data_status(source="cninfo", updated_at="2099-01-01T00:00:00Z", cache_hit=True),
    )

    signals_status = service._build_signals_status(risk_status=risk_status)

    assert signals_status.status == "partial"
    assert signals_status.error_message == "price retry pending"
    assert signals_status.source_metadata.selected_source == "derived"
    assert signals_status.source_metadata.returned_sources == ["derived"]
    assert signals_status.source_metadata.selection_reason == "signals_rollup"
